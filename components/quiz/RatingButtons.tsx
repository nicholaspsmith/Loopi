      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-2 sm:gap-3">
        {ratings.map(({ value, label, color }) => (
          <button
            ...
            className={`px-3 py-3 sm:px-4 sm:py-3 min-h-[48px] ${color} text-white text-sm sm:text-base font-medium rounded-lg transition-colors duration-200 disabled:opacity-50 disabled:cursor-not-allowed focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-white dark:focus:ring-gray-800`}
          >
            {label}
          </button>
        ))}
